...
export async function GET(request) {
  ...
  const body = {
    filter: {
      and: [
        ...
        { property: 'Files & media', files: { is_not_empty: true } },
  ...
  const items = (data.results || [])
    .map((page) => {
      ...
      const filesProp = props['Files & media']?.files || [];
      ...
      const formatos = (props['Formato de Publicación']?.multi_select || []).map((o) => o.name);
